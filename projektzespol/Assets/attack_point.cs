...
public class attack_point : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (tmp <= 0) transform.localPosition = new Vector3(transform.localPosition.x,0.55f+(Mathf.Sin(tmp*Mathf.Deg2Rad) *offset), transform.localPosition.z);
        else transform.localPosition = new Vector3(transform.localPosition.x, 0+Mathf.Cos(tmp*Mathf.Deg2Rad)/2, transform.localPosition.z);
